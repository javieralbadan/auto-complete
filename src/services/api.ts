...
export const fetchSuggestions = async ({ query }: SearchProps): Promise<FetchSuggestionsResult> => {
    if (!query) {
	...
	try {
		...
		const catchedError = error instanceof Error ? error.message : "An uknown error occurred";
